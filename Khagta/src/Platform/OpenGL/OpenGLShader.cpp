...
namespace Khagta
{
	...
	OpenGLShader::OpenGLShader(const std::string& filepath)
	{
		...
		auto count = lastSlash == std::string::npos ? filepath.size() - lastSlash : lastDot - lastSlash;
	...
	std::string OpenGLShader::ReadFile(const std::string& filepath)
	{
		...
		{
			KG_CORE_ERROR("Could not close file: '{0}'", filepath);
		}
		return result;
	}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			std::string type = source.substr(begin, eol - begin); //eol = Endl of Line
	...
	void OpenGLShader::Compile(std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		KG_CORE_ASSERT(shaderSources.size() <= 2, "We Only Support two Shaders for now");
		...
		if (isLinked == GL_FALSE)
		{
			...
			KG_CORE_ERROR("{0}", infoLog.data());
			KG_CORE_ASSERT(false, "Shader linking failure!");
			return;
		}
